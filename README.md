...
Site for Inspire Campaigns, a Vermont video and marketing studio (Javi Matos Rodriguez and Ant Hylton). Plain HTML/CSS/JS, deployed on Vercel with no build step.
...
- **Colour modes:** light, dark and "Trippy" (pop-art), picked from the emoji dropdown in the nav and saved in `localStorage` (`ic-theme`). First visit follows the system light/dark setting. Tokens live at the top of `styles.css` under `[data-theme]`; sections pick a red / yellow / blue tone with `data-tone`.
